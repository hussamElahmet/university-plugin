...
$(document).ready(function(){
	...
	term_table = $('#term_table').DataTable({
	 	responsive:true,
	 	ajax:{
	 		url:mBaseUrl+'/wp-json/university/v1/get_faculty_all/',
	 		type:'POST'
	 	},
	 	columns:[
	 		{data:'faculty_name'},
...
function getPostData () {
	...
	var faculty_id = $('#operation_id').val();
	var faculty_name = $('#term_name').val();
...
function setData (data) {
	emptyFields();
	$('#operation_id').val(data.faculty_id);
	$('#term_name').val(data.faculty_name);
}
...
function saveTerm(data) {
	$.ajax({
		type: 'POST',
		url:mBaseUrl+'/wp-json/university/v1/create_faculty',
		data: data,
...
function updateTerm(data){
	$.ajax({
		type: 'POST',
		url:mBaseUrl+'/wp-json/university/v1/update_faculty',
		data: data,
		success:function(result) {
			console.log(result);
			emptyFields();
...
function deleteTerm(faculty_id){
	var ok = confirm("متأكد من الحذف");
	if (ok) {
		$.ajax({
			type: 'POST',
			url:mBaseUrl+'/wp-json/university/v1/delete_faculty',
			data: {
				faculty_id:faculty_id
			},
			success:function(result) {
...
function getTerm(faculty_id){
	$.ajax({
		type: 'POST',
		url:mBaseUrl+'/wp-json/university/v1/get_faculty',
		data: {
			faculty_id:faculty_id
		},
		success:function(result) {
