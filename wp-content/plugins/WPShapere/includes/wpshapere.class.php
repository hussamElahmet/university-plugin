...
if (!class_exists('WPSHAPERE')) {
  ...
  class WPSHAPERE
  {
	...
	function __construct()
	{
      ...
      //add_action( 'current_screen', array($this, 'vsp_admin_page_block_handler'), 5 );
	}
  ...
  function wps_welcome_msg() {
    if(isset($_GET['status']) && $_GET['status'] == "wps-activated") {
       echo '<h1 style="line-height: 1.2em;font-size: 2.8em;font-weight: 400;">' . __('Welcome to WPShapere ', 'wps') . '</h1><br />';
    }
  }
  ...
  */
	...
	{
    //echo '<pre>'; print_r($nowpage); echo '</pre>';
    wp_enqueue_script('jquery');
    wp_enqueue_style( 'dashicons' );
    wp_enqueue_style('fontAwesome', WPSHAPERE_DIR_URI . 'assets/font-awesome/css/font-awesome.min.css', '', WPSHAPERE_VERSION);
    wp_enqueue_style('lineIcons', WPSHAPERE_DIR_URI . 'assets/lineicons/lineicons.min.css', '', WPSHAPERE_VERSION);
    if($nowpage == 'toplevel_page_wpshapere-options') {
      wp_enqueue_script( 'wps-livepreview', WPSHAPERE_DIR_URI . 'assets/js/aof-live-preview.min.js', array( 'jquery' ), '', true );
    }
	}
...
function wps_video_frame_css()
{
  ...
  public function hideupdateNotices() {
  	...
  	/**
  	 * VSP Admin Page Block Handler.
  	 *
  	 * @param $cscreen
  	 */
  	function vsp_admin_page_block_handlersss( $cscreen ) {


        $pages = array('fusion-builder-options', 'wp-hide','post_type=product');
    		//$pages        = array_keys( $vsp_page_blocker );
    		$script_name  = str_replace( '/wp-admin/', '', $_SERVER['SCRIPT_NAME'] );
    		$query_string = ( isset( $_SERVER['QUERY_STRING'] ) ) ? $_SERVER['QUERY_STRING'] : '';
    		$block        = false;

    		if ( in_array( $cscreen->id, $pages, true ) ) {
    			$block = $cscreen->id;
    		}

    		if ( isset( $_REQUEST['page'] ) && ! empty( $_REQUEST['page'] ) && in_array( $_REQUEST['page'], $pages, true ) ) {
    			$block = $_REQUEST['page'];
    		}

    		if ( isset( $query_string ) && ! empty( $query_string ) && in_array( $query_string, $pages, true ) ) {
    			$block = $query_string;
    		}

    		if ( ! empty( $script_name . '?' . $query_string ) && in_array( $script_name . '?' . $query_string, $pages, true ) ) {
    			$block = $script_name . '?' . $query_string;
    		}
